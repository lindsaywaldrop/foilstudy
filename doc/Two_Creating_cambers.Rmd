...
This notebook will guide you through creating the necessary airfoils to run the foil study. It uses the R scripts found in `./src/r-scripts` and the parameter list of cambers in `./data/camber_list.csv` to alter the airfoil `./data/AS6091.csv` to produce airfoils with different cambers, the output will be located in `./data/`. 

Please ensure that the notebook is being knitted through the project directory: select the drop down arrow next to "Preview" and then "Knit Directory" and select "Project Directory."
...
```{r}
write.table(new_foil, file = "./data/airfoils/ASnew.dat", row.names = F, col.names = F)
```


### Produce all airfoil files

Please source the `calculate_cambers.R` script located in the `./src` folder. The following line can do so if it is uncommented:
...
Now that all the airfoils have been created, you can move on to running simulations in MATLAB XFOIL! In MATLAB, access the `Three_xfoil_step_MATLAB.mlx` file and run the code. Since XFOIL is not implemented in R, we include results from simulations run in XFOIL as part of this repository. If you would like to skip this step, you can proceed to `Four_Visualizing_results.Rmd`. 