...
all.equal(poly_roots_matlab[,1], poly_roots)
```
...
all.equal(param_combo_subset, param_combo_subset_matlab, check.attributes = F)
```
...
alpha_mat_matlab <- as.matrix(read.csv("./doc/alpha_new.csv", header = F))
diff <- test_matlab_match(alpha_mat, alpha_mat_matlab, 1e-14)
all.equal(diff, 1:84)
all.equal(alpha_mat, alpha_mat_matlab, check.attributes = F)
```
...
all.equal(info_mat,info_mat_matlab, check.attributes =F)
```
...
matlab_Y <- matlab_Y[, 1]
abs(Y - matlab_Y) < 1e-13
all.equal(Y, matlab_Y)
```
...
matlab_temp_mat <- as.matrix(read.csv("./doc/temp_mat.csv", header=F))
all.equal(temp_mat, matlab_temp_mat, check.attributes = F)
```
...
matlab_sCoeffs <- as.matrix(read.csv("./doc/s_coeffs.csv", header = F))
...
print(paste0("s", c(12, 13, 23), ": ", round(sobols[["s2nd"]][!is.na(sobols[["s2nd"]])], digits = 4)))
