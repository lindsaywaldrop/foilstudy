...
unscaled_params <- matrix(NA, nrow = 343, ncol = 3)
unscaled_params[,2] <- ((train_gpc_params[,2]/max(aoa_range))*2)-1
unscaled_params[,3] <-  round(((train_gpc_params[,3] - min(camber_range))/
                                 (max(camber_range) - min(camber_range)))*2 - 1, 
                              digits = 4)
unscaled_params[,1] <- (train_gpc_params[,1] - min(Re_range))/
...
test_set <- unscaled_params[order(param_reorder),]
...
test_set <- cbind(test_set, training_gpc_data$output, training_gpc_data$sim)
# Remove failed simulations
training_gpc_data <- training_gpc_data[!is.nan(training_gpc_data$output), ]
training_gpc_data <- as.data.frame(test_set[!is.nan(test_set[,4]),])
colnames(training_gpc_data) <- c("Re", "aoa", "camber", "output", "sim")
```

```{r}

#unscaled_params <- unscaled_params[order(unscaled_params[,4]),]
```
...
```{r}
gpc_info_matlab <- R.matlab::readMat("./gPC/gPC_Info.mat")
param_combo_matlab <- gpc_info_matlab$param.combo
info_mat_matlab <- gpc_info_matlab$INFO.MAT
alpha_mat_matlab <- gpc_info_matlab$alphaMAT
gpc_train_data_matlab <- R.matlab::readMat("./gPC/gPC_Training_Dataset.mat")
train_params_matlab <- gpc_train_data_matlab$Train.PARAMS
train_results_matlab <- gpc_train_data_matlab$Training.RESULTS
```
...
train_gpc_params <- train_gpc_params[training_gpc_data$sim,]
# Matlab data
sims_used <- train_results_matlab[,1]
info_mat_matlab <- info_mat_matlab[sims_used,]
...
output_matlab <- train_results_matlab[,2]
```
...
plot_gpc_train <- data.frame("sim" = train_gpc_results$sim, 
                             "gpc_train" = gpc_CL, 
                             "gpc_real" = output_gpc)
