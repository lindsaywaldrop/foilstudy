---
title: "Generalized Polynomial Chaos Primer"
...
This work is a validated translation of Nick Battista's MATLAB code into R. This section steps through the entire process found in `./src/matlab/gpc/USER_RUN_gPC_Expansion.m` that is called as a function in the MATLAB live script `./doc/Generating_points_MATLAB.mlx` in R. 

## Creating gPC parameter sequences
...
Initial parameters: 
 ...
 - `test_flag` will test the results against expected values produced by the original code in MATLAB.`TRUE` should be displayed in the output of each code chunk (at least once) if testing is successful. If you wish to intrinsically validate this code against the MATLAB code, use `1` to test, otherwise `0` will turn testing off. 
...
You can use either the full set `param_combo` or the smaller subset `param_combo_subset` for running your full simulations. 

## Validation: Evaluate and test parameter combinations
...
## Analyzing global sensitivity using Sobol indices
