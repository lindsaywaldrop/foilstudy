...
#### Maybe unneeded ####
sink("./data/camber_files/AS6091_base.dat")
cat("AS6091\ camber=", camber.original, "\n")
sink()

write.table(ASwing, file = "./data/camber_files/AS6091_base.dat",
...
for (i in 1:length(levels.camber)){
  ...
  sink(paste("./data/camber_files/AS6091_cam",i,".dat",sep=""))
  cat("AS6091\ camber=",camber.check,"\n")
  sink()
  #write.table(c("wing"),file="birdwing.dat",append=FALSE,col.names=FALSE,row.names=FALSE,sep="  ")
  write.table(ASnew, file = paste("./data/camber_files/AS6091_cam", i, ".dat", sep = ""), 
              append = TRUE, col.names = FALSE, row.names = FALSE, sep = "  ")
}
